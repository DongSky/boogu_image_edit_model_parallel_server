...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Boogu-Image-Turbo generation script.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="models/Boogu-Image-0.1-Turbo",
        required=True,
    ...
    parser.add_argument(
        "--num_inference_steps", type=int, default=4, help="Number of inference steps."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--text_guidance_scale", type=float, default=1.0, help="Text guidance scale."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_dmd_student_inference",
        type=to_bool,
        default=True,
        help=(
            "Enable DMD student few-step T2I inference via the BooguImageTurboPipeline. "
            "Requires pure T2I inputs (no reference images) and "
            "`--text_guidance_scale=1.0 --image_guidance_scale=1.0 "
            "--empty_instruction_guidance_scale=0.0` (no CFG). "
            "Typically used with a distilled student transformer checkpoint passed "
            "via `--custom_diffusion_transformer_path` and a small `--num_inference_steps` (e.g. 4)."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.001,
        help="Conditioning sigma for the DMD student sigma schedule. Only takes effect when `--use_dmd_student_inference=True`.",
    )
...
def load_pipeline(
    ...
    if args.pretrained_pipeline_name_or_path is not None:
        ...
        elif args.use_dmd_student_inference:
            PipelineClass = BooguImageTurboPipeline
        else:
...
def run(
    ...
    results = pipeline(
        ...
        use_dmd_student_inference=args.use_dmd_student_inference,
