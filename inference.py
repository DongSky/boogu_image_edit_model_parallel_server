...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Number of data-parallel shards. Each invocation processes only one "
            "shard of the input items, allowing several processes (e.g. one per GPU) "
            "to split the same batch. Must be >= 1. Defaults to 1 (no sharding)."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Zero-based index of this shard. Must satisfy 0 <= shard_index < num_shards. "
            "Items i are taken when (i %% num_shards) == shard_index. The original "
            "global index is preserved in the output filename so shards never collide."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional separate device for the MLLM (instruction encoder). "
            "When set and different from --device, the MLLM is placed on this device "
            "while the transformer and VAE stay on --device, splitting parameters "
            "across two GPUs (model parallelism). An accelerate AlignDevicesHook is "
            "attached to the MLLM so the rest of the pipeline keeps working with a "
            "single logical device — inputs are moved to --mllm_device before the "
            "MLLM forward, outputs are moved back to --device afterwards. Useful when "
            "the full FP8 pipeline (~21 GB) doesn't fit on a single card. "
            "Incompatible with the cpu/sequential/group offload flags."
        ),
    )
...
def load_pipeline(
    ...
    use_split_mllm_device = bool(
        args.mllm_device and args.mllm_device != args.device
    )

    if use_split_mllm_device:
        if any(
            [
                args.enable_sequential_cpu_offload_flag,
                args.enable_model_cpu_offload_flag,
                args.enable_group_offload_flag,
                args.enable_inner_devices_manager,
            ]
        ):
            raise ValueError(
                "[Model-Parallel Error]: `--mllm_device` cannot be combined with any "
                "offload strategy or `--enable_inner_devices_manager`. Got: "
                f"mllm_device={args.mllm_device}, device={args.device}, "
                f"sequential_cpu_offload={args.enable_sequential_cpu_offload_flag}, "
                f"model_cpu_offload={args.enable_model_cpu_offload_flag}, "
                f"group_offload={args.enable_group_offload_flag}, "
                f"inner_devices_manager={args.enable_inner_devices_manager}."
            )

        from accelerate.hooks import AlignDevicesHook, add_hook_to_module

        print(
            f"[Pipeline Loader]: Model-parallel placement — "
            f"transformer/VAE on {args.device}, MLLM on {args.mllm_device}."
        )
        pipeline.transformer.to(args.device)
        pipeline.vae.to(args.device)
        if (
            getattr(pipeline, "prompt_embedding", None) is not None
        ):
            pipeline.prompt_embedding.to(args.device)
        pipeline.mllm.to(args.mllm_device)

        # AlignDevicesHook with offload=False (default) keeps weights resident on
        # `execution_device`. With io_same_device=True, the hook moves inputs to the
        # MLLM device before forward and moves outputs back to the caller's device,
        # so the rest of the pipeline can keep operating with `device=args.device`.
        add_hook_to_module(
            pipeline.mllm,
            AlignDevicesHook(
                execution_device=torch.device(args.mllm_device),
                io_same_device=True,
            ),
        )
    elif args.enable_inner_devices_manager:
        if not all(
            [
...
def main(args: argparse.Namespace, root_dir: str) -> None:
    ...
    # Data-parallel sharding: pick only the items that belong to this shard.
    if args.num_shards < 1:
        raise ValueError(
            f"--num_shards must be >= 1, got {args.num_shards}."
        )
    ...
    global_indices = list(range(len(instructions)))
    if args.num_shards > 1:
        shard_mask = [
            (idx % args.num_shards) == args.shard_index for idx in global_indices
        ]
        global_indices = [
            idx for idx, keep in zip(global_indices, shard_mask) if keep
        ]
        instructions = [instructions[idx] for idx in global_indices]
        if input_images is not None:
            input_images = [input_images[idx] for idx in global_indices]
        if input_image_paths is not None:
            input_image_paths = [input_image_paths[idx] for idx in global_indices]
        print(
            f"[Shard {args.shard_index}/{args.num_shards}] processing "
            f"{len(global_indices)} of original items (global indices: {global_indices})."
        )

    if len(instructions) == 0:
        print(
            f"[Shard {args.shard_index}/{args.num_shards}] has no items to process. Exiting."
        )
        return
    ...
    # Save output images. When sharding, use the original global index in the filename
    # so multiple shards writing to the same output_image_path never collide.
    os.makedirs(os.path.dirname(args.output_image_path), exist_ok=True)
    image_name, ext = os.path.splitext(args.output_image_path)
    if args.num_shards > 1:
        # One file per generated image, named by global index. Skip the collage
        # because each shard only sees a subset of the full batch.
        for local_i, image in enumerate(results.images):
            global_idx = global_indices[local_i]
            out_path = f"{image_name}_{global_idx}{ext}"
            image.save(out_path)
